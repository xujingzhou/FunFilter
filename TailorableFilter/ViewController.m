...
//  TailorableFilter
//
//  Created by Johnny Xu(徐景周) on 7/13/15.
...
@interface ViewController ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate, PBJVideoPlayerControllerDelegate, ThemeScrollViewDelegate, SKStoreProductViewControllerDelegate, IASKSettingsDelegate, UIPopoverControllerDelegate>
{
...
@property (nonatomic, strong) UIButton *undoCroppableButton;

@property (nonatomic, strong) ThemeScrollView *themeScrollView;
...
- (void)showVideoPlayView:(BOOL)show
{
    if (show)
    {
        ...
        _undoCroppableButton.hidden = NO;
    ...
    else
    {
        ...
        _undoCroppableButton.hidden = YES;
    }
}

- (void)showThemeScrollView
{
    self.themeScrollView.hidden = NO;
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
                         ...
                         _themeScrollView.frame = CGRectMake(0, navHeight + statusBarHeight, CGRectGetWidth(self.view.frame), height);
                     } completion:^(BOOL finished) {
                         
                     }];
}

- (void)hiddenThemeScrollView
{
    CGFloat statusBarHeight = 0; //iOS7AddStatusHeight;
    CGFloat navHeight = CGRectGetHeight(self.navigationController.navigationBar.bounds);
    CGFloat height = 150;
    [UIView animateWithDuration:0.3
                     animations:^{
                         ...
                         _themeScrollView.frame = CGRectMake(CGRectGetWidth(self.view.frame), navHeight + statusBarHeight, CGRectGetWidth(self.view.frame), height);
                     } completion:^(BOOL finished) {
...
- (void)reCalcVideoSize:(NSString *)videoPath
{
    ...
    CGFloat width = 50;
    _closeVideoPlayerButton.frame = CGRectMake(CGRectGetMinX(_videoContentView.frame) - width/2, CGRectGetMinY(_videoContentView.frame) - width/2, width, width);
    _undoCroppableButton.frame = CGRectMake(CGRectGetMaxX(_videoContentView.frame) - width/2, CGRectGetMinY(_videoContentView.frame) - width/2, width, width);
...
- (void)createRecommendAppView
{
    ...
    // Demo button
    CGFloat widthButton = 100, heightButton = 60;
    _demoButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.view.frame)/2 - widthButton/2, CGRectGetHeight(self.view.frame) - heightButton, widthButton, heightButton)];
...
- (void)createVideoPlayView:(UIView *)parentView
{
    ...
    // Undo croppable button
    UIImage *imageUndo = [UIImage imageNamed:@"undo"];
    _undoCroppableButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_videoContentView.frame) - width/2, CGRectGetMinY(_videoContentView.frame) - width/2, width, width)];
    [_undoCroppableButton setImage:imageUndo forState:(UIControlStateNormal)];
    [_undoCroppableButton addTarget:self action:@selector(handleResetButton:) forControlEvents:UIControlEventTouchUpInside];
    [parentView addSubview:_undoCroppableButton];
    _undoCroppableButton.hidden = YES;
}
...
- (void)createNavigationItem
{
    NSString *fontName = GBLocalizedString(@"FontName");
    CGFloat fontSize = 18;
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:GBLocalizedString(@"Theme") style:UIBarButtonItemStylePlain target:self action:@selector(handleThemeButton:)];
...
- (void)createThemeScrollView
{
    CGFloat statusBarHeight = 0; //iOS7AddStatusHeight;
    CGFloat navHeight = CGRectGetHeight(self.navigationController.navigationBar.bounds);
    CGFloat height = 150;
    _themeScrollView = [[ThemeScrollView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.view.frame), navHeight + statusBarHeight, CGRectGetWidth(self.view.frame), height)];
    _themeScrollView.backgroundColor = [UIColor clearColor];
    [self.view addSubview:_themeScrollView];
    
    [self.themeScrollView setDelegate:self];
    [self.themeScrollView setCurrentSelectedItem:0];
    [self.themeScrollView scrollToItemAtIndex:0];
    self.themeScrollView.hidden = YES;
}
...
- (void)viewDidLoad
{
    ...
    // Test in simulator
//    NSString *demoVideoPath = getFilePath(@"IMG_0916.m4v");
//    [self setPickedVideo:getFileURL(demoVideoPath)];
}
...
- (void)handleThemeButton:(UIBarButtonItem *)sender
{
    NSLog(@"handleThemeButton");
    ...
    if (_croppableView)
    {
        ...
        if (pointsPath && [pointsPath count] > 0)
        {
            ...
            for (NSUInteger i = 0; i < [pointsPath count]; ++i)
            {
                ...
                {
                    // If is closed curve, skip this point
